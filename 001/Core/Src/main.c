...
int main(void)
{
  ...
  while (1)
  {
	...
	/*
	 //question 1
	 //use variable i, j at line 118,119
	 //-> on
	GPIOF->ODR = 0x00;
	...
	*/
	...
	// question 3
		...
		{
			HAL_Delay(350);
			GPIOF->ODR = GPIOF->ODR | ((0x01 << 0) | (0x01 << 1) | (0x01 << 4) | (0x01 << 5));
			HAL_Delay(350);
			
			GPIOF->ODR = GPIOF->ODR ^ 0xff;
			HAL_Delay(350);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : pb1_Pin pb2_Pin pb3_Pin pb4_Pin
                           pb5_Pin pb6_Pin pb7_Pin pb8_Pin */
