...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  DWT_Delay_Init();
	GPIOA->MODER &= ~0x03;  // change PA1 00
	GPIOA->MODER |= 0x01;  // change PA1 01
	GPIOA->ODR &= ~0x01;  // change PA1 00
  /* USER CODE END 2 */
			lnitLcd();
  ...
  {
    /* USER CODE END WHILE */
		sprintf(arr,"%02d : %02d : %02d", hh, mm, ss);
		Locate(0,4);
		DispString(arr);
